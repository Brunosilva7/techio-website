( function( $ ){
  $( document ).ready( function(){
    $( '.storepress-btn-get-started' ).on( 'click', function( e ) {
        e.preventDefault();
        $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
        $.post( storepress_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
      ...
      $.ajax( ajaxurl,
        {
          type: 'POST',
          data: {
            action: 'storepress_dismissed_notice_handler',
            type: type,
          }
        } );
    } );
